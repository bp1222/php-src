...
void zend_compile_compare_op(znode *result, zend_ast *ast) /* {{{ */
{
	...
	zend_op *opline, *opline_jmpz;
	uint32_t opcode, opnum_jmpz;
	zend_bool valid = 0;

	if (right_ast->kind == ZEND_AST_COMPARE_OP && !(ast->attr & ZEND_COMPARE_EQUALITY)) {
	...
	// Get the next opnum, before compiling right.  Since the right node can
	// contain codes itself that we may need to jump over.  So track here
	// and before we emit our jmp op; ensure the opnum is the same otherwise
	// we need to inject our op at this location
	if (left_ast->kind == ZEND_AST_COMPARE_OP) {
	...
	if (left_node.op_type == IS_CONST && Z_TYPE(left_node.u.constant) == IS_FALSE) {
		...
		opcode = ast->attr & ~ZEND_COMPARE_MASK;
		zend_compile_expr(&right_node, right_ast);
		...
		if (left_node.op_type == IS_CONST && right_node.op_type == IS_CONST) {
			...
			if (left_ast->kind == ZEND_AST_COMPARE_OP) {
				opline_jmpz = zend_emit_op_at(opnum_jmpz, NULL, ZEND_JMPZ_EX, &left_node, NULL);
			}
			...
			do {
				if (opcode == ZEND_IS_EQUAL || opcode == ZEND_IS_NOT_EQUAL) {
					if (left_node.op_type == IS_CONST) {
						if (Z_TYPE(left_node.u.constant) == IS_FALSE) {
							...
							opline = zend_emit_op_tmp(result, opcode, &right_node, NULL);
							break;
						...
						if (Z_TYPE(right_node.u.constant) == IS_FALSE) {
							opcode = (opcode == ZEND_IS_NOT_EQUAL) ? ZEND_BOOL : ZEND_BOOL_NOT;
							opline = zend_emit_op_tmp(result, opcode, &left_node, NULL);
							break;
				...
				if (ast->attr & ZEND_COMPARE_GREATER) {
					opline = zend_emit_op_tmp(result, opcode, &right_node, &left_node);
					opline->extended_value |= ZEND_COMPARE_GREATER;
				} else {
					opline = zend_emit_op_tmp(result, opcode, &left_node, &right_node);
				}
			} while (0);
